...
function getTypeWiseProduct (productTypeSlug, type)
{
    if (type === 'lowToHigh')
    {
        toUrl = baseurl+'sub-category-product/'+productTypeSlug+'/lth';
    } else if (type === 'highToLow')
    {
        toUrl = baseurl+'sub-category-product/'+productTypeSlug+'/htl';
    } else if (type === 'cat-sub') {
        toUrl = baseurl+'sub-category-product/'+productTypeSlug;
    }
    $.ajax({
...
$('#shortByPrice').submit(function (){
    ...
    if (maxPrice === '' || maxPrice === '')
    {
        ...
        $.ajax({
            url: baseurl+'sub-category-product/sort-by-price',
            method: 'POST',
            dataType: 'JSON',
            data: data,
            beforeSend: function (){
...
function getBrandWiseCatProduct(productTypeSlug, brandSlug)
{
    event.preventDefault();
    $.ajax({
        url: baseurl+'sub-category-product/'+productTypeSlug+'/brand/'+brandSlug,
        method: 'GET',
        dataType: 'JSON',
        beforeSend: function (){
            $('.print-products-cat').append(loaderDiv);
