...
function getTypeWiseProduct (productTypeSlug, type)
{
    if (type === 'lowToHigh')
    {
        toUrl = baseurl+'brand-product/'+productTypeSlug+'/lth';
    } else if (type === 'highToLow')
    {
        toUrl = baseurl+'brand-product/'+productTypeSlug+'/htl';
    } else if (type === 'cat-sub') {
        toUrl = baseurl+'brand-product/'+productTypeSlug;
    }
    $.ajax({
        url: toUrl,
        method: 'GET',
        ...
        success: function (data){
            if (data.products === 'failed')
            {
                ...
                if (data.products.length >0)
                {
                    ...
                    $("#subCategorySlug").val(data.productTypeSlug);
                    appendData(data, productTypeSlug);
...
$('#shortByPrice').submit(function (){
    ...
    if (maxPrice === '' || maxPrice === '')
    {
        ...
        $.ajax({
            url: baseurl+'brand-product/sort-by-price',
            method: 'POST',
            dataType: 'JSON',
            data: data,
            beforeSend: function (){
...
// get products by brand name click
$(document).on('click','#brandSlug', function (){
...
function appendData(data,productTypeSlug)
{
    ...
    $.each(data.brands, function (key, brand){
        ...
        a   += '<a href="javascript:void(0)" id="brandSlug" data-slug="'+brand.slug+'" style="cursor: pointer" class="dropdown-item p-2">'+brand.name+'</a>';
    });
    brandLoop.html(a);
